...
def _post_bytes(url: str, payload: dict, headers: dict, timeout: int = 120) -> bytes:
    data = json.dumps(payload).encode("utf-8")
    request = urllib.request.Request(url=url, data=data, method="POST")
    for key, value in headers.items():
        request.add_header(key, value)
...
def _build_messages(transcript: str) -> list:
    system_msg = (
        "You are an expert Reddit content creator. "
        ...
        "reddit_body: 2-4 paragraphs of engaging conversational text. "
        "image_prompt: descriptive text-to-image prompt. "
...
def _fallback_content(transcript: str) -> Dict[str, str]:
    snippet = transcript.strip() or "a quick thought"
    return {
        "reddit_title": f"Quick thought: {snippet[:80]}",
        "reddit_body": f"I just wanted to share this: {snippet}",
        "image_prompt": "Abstract colorful digital art illustration",
    }
...
def _generate_via_openrouter(transcript: str) -> Dict[str, str]:
...
# ---------------------------------------------------------------------------
# Image generation: chain of free providers
# ---------------------------------------------------------------------------
...
def _generate_image_huggingface(prompt: str) -> str:
    """Use HuggingFace Inference Providers via the official client."""
    if not HF_TOKEN:
...
def _generate_image_pollinations(prompt: str) -> str:
    """Use Pollinations.ai (free, no auth)."""
